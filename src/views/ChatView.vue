    ...
    <div v-else-if="!activeChatId">
      <section class="section-gap">
        <div class="section-title-wrap">
          ...
          <h1 class="page-title">
            Chat <span v-if="totalUnread > 0" class="unread-badge">{{ totalUnread }}</span>
          </h1>
        </div>
      </section>
    ...
    <div v-else class="active-chat">
      <div class="active-header">
        ...
        <div class="active-avatar">
          {{ (activeChat?.name || activeChat?.other_user?.name || '?')[0].toUpperCase() }}
        </div>
...
.chat-list {
  @apply flex flex-col gap-2.5 sm:gap-3;
}
.chat-item {
  @apply flex items-center gap-3 sm:gap-4 rounded-2xl p-2.5 sm:p-4 cursor-pointer;
  border: 2px solid #111;
...
.chat-time {
  @apply text-[9px] sm:text-[10px] text-black/40 flex-shrink-0 uppercase tracking-wide font-bold whitespace-nowrap;
}
.chat-preview {
...
.chat-unread-pill {
  @apply min-w-[22px] h-[22px] px-1.5 rounded-md flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0;
  background: linear-gradient(135deg, #c1121f, #780000);
}
...
.active-header {
  @apply flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-4 pb-3 sm:pb-4 border-b border-black/5;
}
.back-btn {
  @apply w-9 h-9 rounded-lg flex items-center justify-center text-black/70 hover:bg-black/5 hover:text-[#C1121F] transition-all duration-200 active:scale-90;
}
.active-avatar {
...
.msg-bubble {
  @apply max-w-[75%] px-4 py-2.5 text-sm leading-snug transition-transform duration-200;
}
.msg-mine {
...
.send-btn {
  @apply w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-white flex-shrink-0;
  background: linear-gradient(135deg, #c1121f, #780000);
  border: 2px solid #111;
  box-shadow: 2px 2px 0 0 #111;
  transition:
