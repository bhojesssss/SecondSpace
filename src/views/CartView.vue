    ...
    <template v-else>
      <section class="section-gap">
        <div class="section-title-wrap">
          ...
          <h1 class="page-title">
            Keranjang Saya <span class="page-title-count">({{ totalQty }} items)</span>
          </h1>
        </div>
      </section>
...
function changeQty(item, newQty) {
  ...
  _qtyTimers[item.id] = setTimeout(async () => {
    try {
      await api.patch(`/cart/${item.id}`, { qty: newQty })
    } catch (e) {
      console.error('Gagal update qty:', e.message)
    }
  }, 500)
}
...
.empty-icon {
  @apply w-20 h-20 rounded-full flex items-center justify-center mb-5 transition-transform duration-300 hover:scale-110;
  background: linear-gradient(135deg, #c1121f, #780000);
...
.cart-items {
  @apply flex-1 w-full flex flex-col gap-3 sm:gap-4;
}
.cart-item {
...
.item-thumb {
  @apply w-20 h-20 sm:w-32 sm:h-32 flex-shrink-0 rounded-xl overflow-hidden;
  background: #fdf0d5;
  border: 2px solid #111;
}
.item-details {
...
.qty-btn {
  @apply w-7 h-7 rounded-lg flex items-center justify-center text-black hover:text-white hover:bg-[#C1121F] disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-black transition-colors duration-200;
}
.qty-num {
